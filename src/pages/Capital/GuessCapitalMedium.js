...
const GuessCapitalMedium = () => {
  ...
  const handleAnswer = (selectedOption) => {
    setSelectedAnswer(selectedOption); // Réponse sélectionnée

    if (selectedOption.capital === correctAnswer.capital) {
      setScore(score + 1); // Augmenter le score si la réponse est correcte
      playSound("/assets/sounds/win.mp3"); // Son pour bonne réponse
    } else {
      playSound("/assets/sounds/loser.mp3"); // Son pour mauvaise réponse
    }
...
export default GuessCapitalMedium;
